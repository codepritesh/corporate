...
if(isset($_POST['submit']))
{
    ...
    $chk=mysql_query("SELECT * FROM `enterpriseloan` where `customer_id`='$custid' and `loan_against_acc`='$loan_against_acc' and `status`='1' ")or die(mysql_error());
    $row=mysql_num_rows($chk);
   /*echo "INSERT INTO `enterpriseloan` set  `introducer`='$intid', `customer_id`='$custid', `name`='$name',
    ...
    $qwe=mysql_query("INSERT INTO `enterpriseloan` set  `introducer`='$intid', `customer_id`='$custid',`plan`='$plan',`intrestrate`='$intrate', `name`='$name',
                     `dob`='$dob', `gender`='$gender', `phone`='$phone',  `village`='$village', `address`='$address', `post`='$post',
                     `pin`='$pin', `dist`='$dist', `photo`='$photo', `idproof`='$idproof', `sign`='$sign', `documents`='$docs',
                     `nominee_name`='$nominee', `nominee_address`='$nominee_address', `nominee_relation`='$nominee_relation',`folio`=9,
...
header("location:enterpriseloan.php?msg=$msg");
?>