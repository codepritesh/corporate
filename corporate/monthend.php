...
<script type="text/javascript">
   function monthend() {
            if (confirm('Are you sure you want end the current month ?'))
            {
	    var date = $("#date").val();
             $.ajax({
                url:"ajax_monthend.php?date="+date,
                success: function(result){
                    if (result.trim()=="OK") {
                        alert("Month End Successfully.");
                    }                
                }
            });
           }
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Montn End</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
             <table style="float: left;width: 90%;text-align: center;">
                <tr>
		  <td>
		      <input type="text" name="collectiondate" required  autocomplete="off" class="form-control" id="date"  placeholder="Date">
		  </td>
                    <td>
                        <input type="button" value="Month End" onclick="return monthend();" />
                    </td>
                </tr>
             </table>
	    </div>
