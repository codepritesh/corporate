...
//echo "SELECT * FROM customer WHERE EXISTS (SELECT 1 FROM transaction WHERE transaction.customerid = customer.customer_id and transaction.date like '$currdate' and transaction.folio_no = '17' )";
$getvendor=mysql_query("SELECT * FROM customer WHERE EXISTS (SELECT 1 FROM transaction WHERE transaction.customerid = customer.customer_id and transaction.date like '$currdate' and transaction.folio_no = '17' and transaction.productfolio = '7'  )")or die(mysql_error());
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Apply Agriculture Loan</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
                <form name="arrange" action="insertagricultureloan.php" method="post" onSubmit="return validate();" enctype="multipart/form-data">
			...
			<tr id="nomdetails">
			    <td>Nominee Address</td><td><textarea name="nomaddress" class="form-control" id="nomaddress" autocomplete="off"></textarea></td>
			    <td>Nominee Relation</td><td><input type="text" name="nomrelation" id="nomrelation" class="form-control" autocomplete="off"></td>
			</tr>
			...
			<tr>
			<td>Plan</td><td><select name="plan" id="plan" required class="form-control">
			    <option value="">--Select Plan--</option>
			    <?php   $plan=mysql_query("select * from `agricultureloan_plan`");
			...
			<td>Rate of Intrest</td><td><input type="text" name="intrate" id="intrate" required readonly class="form-control"></td>
                        ...
                        <tr>
                            <td>Loan Against Which A/c</td><td><select name="loan_against_acc" id="loan_against_acc" required class="form-control">
			    ...
			    <td>Loan Amount</td><td><input type="text" name="loanamt" id="loanamt" onkeyPress="return IsNumeric(event,this.value)" autocomplete="off" required class="form-control"></td>
			</tr>
			...
			<tr id="laccno">
			    <td>Loan Against Account Number</td><td><input type="text" name="loan_against_accno" id="loan_against_accno" readonly class="form-control"></td>
                        ...
                        <tr >
                            ...
                            <td align="center" colspan="4"><input type="submit" name="submit" value="Apply" /></td>
			</tr>
                     </table>
		</form>
