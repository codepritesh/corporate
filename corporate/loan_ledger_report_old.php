    ...
    <div class="box col-md-12">
        <div class="box-inner">
			...
			<table class="table"  style="font-size: 12px;" >
												...
												<?php
												//echo "select tl.* from `transaction_ledger` tl,(SELECT  max(id) as maxid ,LEFT(cal_date , 7) as calmax  FROM `transaction_ledger` WHERE `accountno` = '$id' group by calmax)tld where tl.id=tld.maxid and MONTH(tl.`cal_date`) <= MONTH(CURDATE()) AND  Year(tl.`cal_date`) <= Year(CURDATE())";
												//$demand=mysql_query("select * from `transaction_ledger` where `accountno`='$id' and MONTH(`cal_date`) <= MONTH(CURDATE()) AND  Year(`cal_date`) <= Year(CURDATE())");
													...
													while($resdemand=mysql_fetch_array($demand))
													{
													   ...
													   $i++;	
														...
														if(($i-1)==1){
														 ...
														 // echo $p.'--'.$t.'--'.$r;
														 $dcurint=round(($p*$t*$r)/100);
														 // echo $rdmd['coll_date'].'---'.$rdmd['cal_date'];
			...
			<th>Total OD</th>
                   ...
                   <?php
		//echo "select * from `transaction_ledger` where `accountno`='$id' order by `cal_date`";
      $fetchtra=mysql_query("select * from `transaction_ledger` where `accountno`='$id' order by `cal_date`,`id` ");
