...
else
{
	...
	$voucher=htmlentities($_POST['voucher']);
        $paymentmode=htmlentities($_POST['paymentmode']);
        $d1=htmlentities($_POST['d1']);
        $d2=htmlentities($_POST['d2']);
        $d3=htmlentities($_POST['d3']);
        $tid=time();
        $date=date("Y-m-d");
	/*echo "insert into `transaction` set `transactionid`='$tid'
                    , `amount`='-$amount',`details`='$perticulars',`mode_of_payment`='$paymentmode'
                    ,`date`='$date',`chq_dd_no`='$d1',`chq_dd_bank_name`='$d2',`chq_dd_date`='$d3',`transaction`='expence'";*/
      mysql_query("insert into `transaction` set `transactionid`='$tid'
                    , `amount`='-$amount',`details`='$perticulars',`mode_of_payment`='$paymentmode'
                    ,`date`='$date',`chq_dd_no`='$d1',`chq_dd_bank_name`='$d2',`chq_dd_date`='$d3',`transaction`='expence',`voucher`='$voucher'");
    $msg="Successfully Submited...";
    header("location:extraexpences.php?msg=$msg");
    }
?>
<script>
    function del(args)
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Extra Expence</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
                <form name="expences" action="" method="post" onSubmit="return validate();">
                    <table style="float: left;width: 100%;">
                    <tr>
                            <td>Perticulars</td><td>
                            <select name="perticulars" required class="form-control">
                                <option></option>
                                <?php
                                $fetch=mysql_query("select * from `expencestype`");
                            ...
                            <td>Amount</td><td><input type="text" name="amount" id="amount" class="form-control" style="autocomplete:off;" required></td>
		    </tr>
                    <tr>
                            <td>Voucher no</td><td><input type="text" name="voucher" id="voucher" value=""  onKeyPress="return numbersonly(event);" style="autocomplete:off;"  class="form-control" required ></td>
			    <td>Mode Of Payment</td><td><select id="paymentmode" name="paymentmode" required class="form-control">
