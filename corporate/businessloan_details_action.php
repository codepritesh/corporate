...
if(isset($_POST['allow']))
{
    ...
    if($loan!="" && $loan>0)
    {
        /*echo "update `businessloan` set `is_approved`='1',`loangiven`='$loan',`intrestrate`='$intrate',
		`amount_topay`='$loan',`loan_given_date`='$disdate', `last_refund_date`='$date' where `id`='$id'";*/
    mysql_query("update `businessloan` set `is_approved`='1',`loangiven`='$loan',`intrestrate`='$intrate',
	...
	header("location:loan_despatch.php?table=businessloan");
        ...
        header("location:businessloan_details.php?id=$id&msg=$msg");
    }
}
if(isset($_POST['reject']))
{
    $id=htmlentities($_POST['loanid']);
    //echo "update `businessloan` set `is_approved`='2',`loan_given_date`='$date' where `id`='$id'";
//mysql_query("update `businessloan` set `is_approved`='2',`loan_given_date`='$date' where `id`='$id'") or die(mysql_error());
mysql_query("delete from `businessloan` where `id`='$id'") or die(mysql_error());
header("location:loan_approve.php?table=businessloan");
}
?>