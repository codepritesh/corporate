...
if(isset($_POST['submit']))
{
	...
	if($cntledger==0){
		...
		$ledgerquery="update `transaction_ledger` set `outstanding`='$outstanding',`b_od_int`='$b_od_int',`b_od_pri`='$b_od_pri',
		    ...
		    `a_od_int`='$a_od_int',`a_od_pr`='$a_od_pr',`a_tot_od`='$a_tot_od',`collection`='$_POST[refund]',`coll_date`='$crdate'
		    where `accountno`='$loan_accno' and `cal_date`='$cal_dt'";
		
	}else{
		$ledgerquery="insert into `transaction_ledger` set `accountno`='$loan_accno',`outstanding`='$outstanding',`cal_date`='$cal_dt',
    ...
    $msg="You have successfully repayment your loan amount...Your loan a/c has been closed...";	
	 ...
	 $tableupdate="update  `$table` set `odintrest`='$a_od_int',`odprincipal`='$a_od_pr',`amount_topay`='$outstanding',`last_refund_date`='$crdate',`last_refund_amt`='$_POST[refund]',`od_cal_date`='$cal_dtt',`loancomplited`='$complited' where `loan_accno`='$loan_accno'";
