...
$fetch=mysql_query("select * from `demand_loan` where `id`='$id'");
$res=mysql_fetch_array($fetch);
    $date=date("Y-m-d");
    $time=time();
    
    $fetchloan=mysql_query("select * from `loanaccno`");
$resloan=mysql_fetch_array($fetchloan);
$id1=$resloan['accno']+1;
mysql_query("update `loanaccno` set `accno`='$id1'");
    //echo "update `demand_loan` set `loan_accno`='L$id',`loan_given_date`='$date' where `id`='$id'";
    mysql_query("update `demand_loan` set `loan_accno`='DL$id1',`loan_given_date`='$date',`od_cal_date`='$date' where `id`='$id'")or die(mysql_error());
    
    /*echo "</br>insert into `transaction` set `transactionid`='$time',`customerid`='$res[customer_id]',`accountno`='L$id'
                 ,`amount`='-$loan',`date`='$date',`details`='Demand Loan',`mode_of_payment`='$payment_mode',
                 `chq_dd_no`='$chkno',`chq_dd_bank_name`='$bank',`chq_dd_date`='$chkdt'";*/
    mysql_query("insert into `transaction` set `transactionid`='$time',`customerid`='$res[customer_id]',`accountno`='DL$id1'
                 ,`amount`='-$res[loangiven]',`date`='$date',`details`='Demand Loan',`mode_of_payment`='$payment_mode',
                 `chq_dd_no`='$chkno',`chq_dd_bank_name`='$bank',`chq_dd_date`='$chkdt',`folio_no`='10',`voucher`='$voucher',`type`='loan'")or die(mysql_error());
   // header("location:demandloan_dispatch.php");
   // header("location:demandscheduler.php?id=$id&table=demand_loan");
   header("location:demandledger.php?id=$id&table=demand_loan");
}
$id=$_GET['id'];
?>
<html lang="en">
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Demand Loan Disbursement</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
			    ...
			    <td>Customer Id</td><td><input type="text" name="cid"  value="<?php echo $res['customer_id'];?>" id="cid" class="form-control" required readonly>
