...
$getvendor=mysql_query("SELECT * FROM customer WHERE EXISTS (SELECT 1 FROM transaction WHERE transaction.customerid = customer.customer_id and transaction.date like '$currdate' and transaction.folio_no = '17' and transaction.productfolio = '11'  )")or die(mysql_error());
/*$getvendor=mysql_query("SELECT * FROM customer WHERE EXISTS (SELECT 1 FROM transaction WHERE transaction.customerid = customer.customer_id and transaction.date like '$currdate' and transaction.folio_no = '17' and transaction.productfolio = '11'  )
		       and NOT EXISTS (SELECT * FROM goldloan
                    WHERE goldloan.customer_id = customer.customer_id and goldloan.is_approved='1' and goldloan.loancomplited = 0)")or die(mysql_error());*/
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Apply Gold Loan</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
                <form name="arrange" action="insert_goldloan.php" method="post" onSubmit="return validate();" enctype="multipart/form-data">
			...
			<td>Plan</td><td><select name="plan" id="plan" required class="form-control">
			    ...
			    <?php   $plan=mysql_query("select * from `goldloan_plan`");
			 ...
			 <tr>
			    <td>Loan Amount</td><td><input type="text" name="loanamt" id="loanamt" onkeyPress="return IsNumeric(event,this.value)" required class="form-control"  autocomplete="off"></td>
                        ...
                        <tr >
                            ...
                            <td style="text-align: center;" colspan="3"><input type="submit" name="submit" value="Apply" /></td>
			</tr>
                     </table>
		</form>
