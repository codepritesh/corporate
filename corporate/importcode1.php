...
/*
    ...
    $f=0;
    $nf=0;
    $fetch=mysql_query("select * from `TABLE 60`");	
    while ($data=mysql_fetch_array($fetch))
	{
             $n++;
	   $time=time();
    ...
    /*$a=0;$b=0;$c=0;
     $qwe=mysql_query("SELECT * FROM `transaction` WHERE `id`>=2124 and `id`<=2327");
    while($res=mysql_fetch_array($qwe))
    {
        ...
        $newdate="2016-".$d.'-'.$m;
         ...
         echo "update `transaction` set `date`='$newdate'  WHERE `id`='$res[id]'";
         mysql_query("update `transaction` set `date`='$newdate'  WHERE `id`='$res[id]'")or die(mysql_error());
        }
        else{
           $a++;
    ...
    echo "T=".$c."F=".$b."NF=".$a;
    /******** May **********/
    /*$qwe=mysql_query("SELECT * FROM `transaction` WHERE `id`>=2328 and `id`<=2333");
    while($res=mysql_fetch_array($qwe))
    {
        $date=$res['date'];
        $m=date("m",strtotime($date));
    ...
    $fetch=mysql_query("select * from `TABLE 59`");
   while ($data=mysql_fetch_array($fetch))
{
   $n++;
  $time=time();
   if($data['COL 1']!='')
           ...
           $accno=$pr.$accno;
           $qq=mysql_query("select * from $table where `loan_accno`='$accno'");
           $q=mysql_fetch_array($qq);
           $custid=$q['customer_id'];
           $amt=$q['loangiven'];
           $date=$q['loan_given_date'];
           ...
           echo "<br/>insert into `transaction` set `transactionid`='$time',`customerid`='$custid',`amount`='$lpf',
           `details`='processing fee',`folio_no`='21',`agentid`='$agentid',`date`='$date'";
           
           mysql_query("insert into `transaction` set `transactionid`='$time',`customerid`='$custid',`amount`='$lpf',`details`='processing fee',`folio_no`='21',`agentid`='$agentid',`date`='$date'") or die(mysql_error());
           
           echo "<br/>insert into `transaction` set `transactionid`='$time',`customerid`='$custid',`amount`='-$amt',
           `details`='$det',`folio_no`='$folio',`agentid`='$agentid',`date`='$date',`accountno`='$accno'";
           
           mysql_query("insert into `transaction` set `transactionid`='$time',`customerid`='$custid',`amount`='-$amt',`details`='$det',`folio_no`='$folio',`agentid`='$agentid',`date`='$date',`accountno`='$accno'") or die(mysql_error());
           echo "</br>".$n;
       }
   }
?>