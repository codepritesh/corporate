...
$res=mysql_fetch_array($fetch);
    ...
    if($security!='' && $security>0){
        mysql_query("update `agricultureloan` set `security_amount`='$security' where `id`='$id'")or die(mysql_error());
    ...
    mysql_query("insert into `transaction` set `transactionid`='$time'
                    ...
                    ,`date`='$date',`chq_dd_no`='$d1',`chq_dd_bank_name`='$d2',`chq_dd_date`='$d3'
		    ,`transaction`='income',`voucher`='$voucher'")or die(mysql_error());;
    //header("location:scheduler.php?id=$id&table=agricultureloan");
    header("location:agricultureloan_despatchact.php?id=$id");
}
$id=$_GET['id'];
?>
<html lang="en">
        ...
        <div class="box-inner">
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
			    ...
			    <td>Customer Id</td><td><input type="text" name="cid"  value="<?php echo $res['customer_id'];?>" id="cid" class="form-control" required readonly></td>
                        ...
                        <tr>
                            <?php
			    $ffetch=mysql_query("select * from `processing_fees` where `folio_id`='$res[folio]'")or die(mysql_error());;
			    $rres=mysql_fetch_array($ffetch);
			    $pre=$rres['fees'];
			    $prate=$pre/100;
                            $profee=$res['loangiven']*$prate;
                            ?>
