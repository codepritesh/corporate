...
{
    $currdate=date("Y-m-d");
   // $lastdate=date('Y-m-t',strtotime($currdate));
    $custmerid='<option value=""></option>';
    //$getvendor=mysql_query("select * from `customer`")or die(mysql_error());
//$getvendor=mysql_query("SELECT * FROM customer WHERE NOT EXISTS (SELECT 1 FROM compulsarydeposite WHERE compulsarydeposite.customer_id = customer.customer_id) and EXISTS (SELECT 1 FROM transaction WHERE transaction.customerid = customer.customer_id and transaction.date like '$currdate' and transaction.folio_no = '17' and transaction.productfolio = '1')")or die(mysql_error());
$getvendor=mysql_query("SELECT * FROM customer WHERE EXISTS (SELECT 1 FROM transaction WHERE transaction.customerid = customer.customer_id and transaction.date like '$currdate'
					   and transaction.folio_no = '17' and transaction.productfolio = '1')")or die(mysql_error());
    ...
    //$sqlagent=mysql_query("select * from `agent` where `prodetails`='Compulsory Deposit'")or die(mysql_error());
...
<script>
    ...
    function change(val)
    {
        var skillsSelect = document.getElementById("plan");
        var selectedText = skillsSelect.options[skillsSelect.selectedIndex].text;
        $('#intrest').val(val);
        $('#timeperiod').val(selectedText);
    }
...
<div class="ch-container">
	...
	<li style="float: right;"> <?php echo date("Y-m-d");?></li>
    </ul>
</div>
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Create Compulsary Deposite</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
                <form name="arrange" action="insert_compulsary.php"  onSubmit="return validate();" method="post" enctype="multipart/form-data">
                     <table style="float: left;width: 100%; height: 450px;">
					  <tr>
                        ...
                        <tr>
                            <td>A/c No</td><td><input type="text" name="accno" value="CM<?php echo $raccno;?>" class="form-control" readonly="true" required></td>
                            <td>Guardian Name</td><td><input type="text" name="gname" id="gname" readonly="true" class="form-control"  required></td>
			    <input type="hidden" name="haccno" id="haccno" value="VS<?php echo $raccno;?>"> 
			</tr>
                        <tr>
			    <td>DOB</td><td><input type="text" name="dob" id="dob" readonly="true" class="form-control"  required></td>
                            <td>Gender</td><td><input type="text" name="gender" id="gender" readonly="true" class="form-control"  required>
			</tr>
                        <tr>
			    <td>Phone No.</td><td><input type="text" name="phno" id="phno" readonly="true" class="form-control"  required></td>
                            <td>Address</td><td><input type="text" name="address" id="address" readonly="true" class="form-control"  required></td>
			</tr>
                        <tr>
			    <td>Post</td><td><input type="text" name="post" id="post" readonly="true" class="form-control"  required></td>
                            <td>Pin</td><td><input type="text" name="pin" id="pin" readonly="true" class="form-control"  required></td>
			</tr>
                        <tr>
			    <td>Dist</td><td><input type="text" name="dist" id="dist" readonly="true" class="form-control"  required></td>
                            <td>Age</td><td><input type="text" name="age" id="age" readonly="true" class="form-control"  required></td>
			</tr>
                        <tr>
			    <td>Annual income</td><td><input type="text" name="annual" id="annual" readonly="true" class="form-control" required></td>
			...
			<tr id="nomdetails">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="hidden" name="timeperiod" id="timeperiod" value="">
                                    ...
                                    <?php
                            $fplan=mysql_query("select * from `compulsorysavingscheme`");
                            while($resplan=mysql_fetch_array($fplan))
                            {
                            ?>
                                 <input type="text" name="intrest" id="intrest" value="<?php echo $resplan['interestrate'];?>" readonly="true" class="form-control" required>   
                            <?php }?>
                               
                            </td>
                            <td>Monthly Deposit Amount</td><td>
							    <?php $qwe=mysql_query("select * from `set_deposit_amt` where `scheme_id`='4'");
								$res=mysql_fetch_array($qwe);
							    ?>
								<input type="text" name="deposited_amt" id="deposited_amt" value="<?php echo $res['amount']?>" readonly="true" class="form-control" required>   
							   
						    </td>
			</tr>
                            <!--<td>Maturity Amount</td><td><input type="text" readonly="true" name="" class="form-control" required></td>-->
			    ...
			    <td>Mode Of Payment</td><td colspan="3"><select id="paymentmode" name="payment_mode" required class="form-control">
                        ...
                        <tr>
                           <td>&nbsp;</td>
			   <td><input type="text" name="d1" id="d1" onKeyPress="return numbersonly(event);"  autocomplete='off' class="form-control" placeholder="Cheque/DD Number" style="display: none;"></td>
