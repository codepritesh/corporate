...
//$getvendor=mysql_query("SELECT * FROM customer WHERE EXISTS (SELECT 1 FROM transaction WHERE transaction.customerid = customer.customer_id and transaction.date like '$currdate' and transaction.folio_no = '17' and transaction.productfolio = '6'  )")or die(mysql_error());
$getvendor=mysql_query("SELECT * FROM customer WHERE EXISTS (SELECT 1 FROM transaction WHERE transaction.customerid = customer.customer_id and transaction.date like '$currdate' and transaction.folio_no = '17' and transaction.productfolio = '6'  )
		       and NOT EXISTS (SELECT * FROM grouploan
                    WHERE grouploan.customer_id = customer.customer_id and grouploan.is_approved='1' and grouploan.loancomplited = 0)")or die(mysql_error());
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Apply Group Loan</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
                <form name="arrange" action="insert_grouploan.php" method="post" onSubmit="return validate();" enctype="multipart/form-data">
			...
			<td>Plan</td><td><select name="plan" id="plan" required class="form-control">
			    ...
			    <?php   $plan=mysql_query("select * from `grouploan_plan`");
