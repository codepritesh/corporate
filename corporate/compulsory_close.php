...
else
{
 ...
 $getvendor=mysql_query("select * from `compulsarydeposite` where `status`=0")or die(mysql_error());
    while($resvendor=mysql_fetch_array($getvendor))
    {
	

	$id=$resvendor['id'];
	$qual=$resvendor['acc_no'];
        $custmerid.='<option value='.$id.'>'.$resvendor['acc_no']."(".$resvendor['name'].")".'</option>';
    }
?>
<?php
if(isset($_POST['submit'])){
    ...
    mysql_query("update  `compulsarydeposite` set `status`=1 where `id`='$id'");
    $qwe=mysql_query("select * from `compulsarydeposite` where `id`='$id'");
    ...
    mysql_query("INSERT INTO `transaction` set `transactionid`='$time',`customerid`='$res[customer_id]',`accountno`='$res[acc_no]'
				,`amount`='$rs',`voucher`='$voucher',`date`='$date',`details`='Income'");
}
?>
<!--autocomplete start-->
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Close Compulsary Account</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
                <form name="arrange" action="#"  onSubmit="return validate();" method="post" enctype="multipart/form-data">
                     <table style="float: left;width: 100%; height: auto;">
					  <tr>
			             ...
			             <tr>
                            <td>A/c No</td><td style="width:310px;"><select name="customer" data-placeholder="&nbsp;"  class="form-control chosen-select " id="customer" style="width:307px; padding:4px;height: 100px;border-radius:4px;">
										<option value=""></option>
										<?php echo $custmerid;?>
									   </select>
			    </td>
                            <td>Rs /- </td><td><input type="text" name="rs" id="rs" value="50.00" class="form-control" readonly="true" required style="width:307px;" ></td>
