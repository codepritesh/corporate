...
{
$custmer='<option value=""></option>';
$getvendor=mysql_query("SELECT * FROM agricultureloan where `is_approved`='1' and `loan_accno`!='' and `loancomplited`='0'") or die(mysql_error());
 while($resvendor=mysql_fetch_array($getvendor))
    {
        $id=$resvendor['id'];
        $custmer.='<option value='.$id.'>'.$resvendor['name'].'</option>';
    }
    
    $sqlagent=mysql_query("select * from `agent`")or die(mysql_error());
    //$sqlagent=mysql_query("select * from `agent` where `prodetails`='agriculture'")or die(mysql_error());
...
<script type="text/javascript"> 
	...
	var cid=$(this).val();
        $.ajax({
            url:"agricultureloan_refund_ajax.php?cid="+cid,
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Agriculture Loan Repayment</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
                <form name="arrange" action="agricultureloanrefund_action.php" method="post" onSubmit="return validate();" enctype="multipart/form-data">		
		   <table style="float: left;width: 100%;">
                        <tr>
                            <td colspan="4" style="color: red;font-size: 20px;"><?php if(isset($_GET['msg'])){ echo $_GET['msg']; }  ?></td>
                        </tr>
			<tr>
                            <td>Loan Holders Name</td><td style="width:310px;"><select name="loan" data-placeholder="&nbsp;"  class="form-control chosen-select"  id="loan" style="width:307px; padding:4px;height: 100px;border-radius:4px;">
										<option value=""></option>
										<?php echo $custmer;?>
									</select>
			    <input type="hidden" name="custid" id="custid">
							</td>
                            <td>A/c No</td><td><input type="text" name="loan_accno" id="loan_accno" class="form-control" value="<?php echo $acc_no?>" required readonly></td>
			...
			<tr>
			    ...
			    <td>OD Principal</td><td><input type="text" name="odprincipal" class="form-control" id="odprincipal" required readonly></td>
			    <input type="hidden" name="lastodcaldate" id="lastodcaldate" />
			</tr>
			<tr>
			    <td>Current Intrest </td><td><input type="text" name="intamt" class="form-control" id="intamt" required readonly></td>
			    <td>Current Principal</td><td><input type="text" name="principal" class="form-control" id="principal" readonly></td>
			</tr>
			<tr>
			    <td>Pre-Paid Principal</td><td><input type="text" name="preprincipal" class="form-control" id="preprincipal" readonly></td>
			...
			<tr>
			    ...
			    <td>Repayment Amount</td><td><input type="text" name="refund" class="form-control" id="refund"  onkeyPress="return IsNumeric(event,this.value)" autocomplete="off" ></td>
			</tr>
			<tr>
			    <td>Voucher No</td>
			    <td>
