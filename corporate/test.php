...
<div class="row">
    <div class="box col-md-12">
	    ...
	    <table class="table" style="float: left;width: 50%; font-size: 8px;">
                    <tr>
		...
		<table class="table" style="float: left;width: 50%; font-size: 8px;">
		    ...
		    $obbal=0;
		    $bankamt=0;
		    $fetch=mysql_query("select * from `transaction` group by `date`")or die(mysql_error());
		    while($res=mysql_fetch_array($fetch)){
		    $c=0;
		    $tot=0;
		  ...
		  <tr>
		    ...
		    <tr>
			     ...
			     <th>TOTAL AMO</th>
                    ...
                    <?php
		    $date=date("2015-12-08");
                    $fet=mysql_query("select * from `transaction` where `amount`<'0' and `date`='$date'")or die(mysql_error());
		    while($re=mysql_fetch_array($fet))
                    {
		    $gbyagent=mysql_query("select * from `transaction` where `amount`<'0' and `date`='$date' group by `agentid`");
		    while($res=mysql_fetch_array($fetch)){
		    
		    $ffetch2=mysql_query("select SUM(amount) as `amt`,`folio_no` from `transaction` where `agentid`='$res[agentid]' and `date`='$date'");
		    $rres2=mysql_fetch_array($fetch2);
			...
			?>
		     ...
		     //echo "</br>select * from `transaction` where `date`='$rpayment[date]' and `amount`<'0' and `accountno`like 'L%' ";
                     $fpayment1=mysql_query("select * from `transaction` where `date`='$rpayment[date]' and `amount`<'0' and `folio_no`='$rpayment[folio_no]' ")or die(mysql_error());
		     while($rpayment1=mysql_fetch_array($fpayment1)){
                       $totramt=$totramt+$rpayment1['amount'];
                        //$faname=mysql_query("select * from `agent` where `id`='$rpayment1[agentid]'")or die(mysql_error());
			//$rresaname=mysql_fetch_array($faname);
                        $faname=mysql_query("select * from `customer` where `customer_id`='$rpayment1[customerid]'")or die(mysql_error());
			$rresaname=mysql_fetch_array($faname);
		...
		<table style="float: left;width: 100%;font-size: 12px;">
                    <tr>
                         <td style="text-align: center;"><input type="button" value="Print" onclick="printContent('srch')" /></td>
                    </tr>
	     </table>
	    </div>
