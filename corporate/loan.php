...
//$getvendor=mysql_query("SELECT * FROM customer WHERE EXISTS (SELECT 1 FROM transaction WHERE transaction.customerid = customer.customer_id and transaction.date like '$currdate' and transaction.folio_no = '17' and transaction.productfolio = '12'  )")or die(mysql_error());
$getvendor=mysql_query("SELECT * FROM customer WHERE EXISTS (SELECT 1 FROM transaction WHERE transaction.customerid = customer.customer_id and transaction.date like '$currdate' and transaction.folio_no = '17' and transaction.productfolio = '12'  )
		       and NOT EXISTS (SELECT * FROM loan
                    WHERE loan.customer_id = customer.customer_id and loan.is_approved='1' and loan.loancomplited = 0)")or die(mysql_error());
   while($resvendor=mysql_fetch_array($getvendor))
    {
	$id=$resvendor['customer_id'];
	$qual=$resvendor['introducer'];
        $custmerid.='<option value='.$id.'>'.$resvendor['name']."(".$resvendor['mem_cus_id'].")".'</option>';
    }
...
<head>
   ...
   <script>
	 ...
	 $("#plans").change(function(){
	    var plid=$(this).val();
	    $.ajax({
            url:"ajax_getloanrate.php?id="+plid,
            success: function(result)
	    {
	    var r=result.split("|"); 
            $('#intrate').val(r[0]);
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Apply Daily Loan</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
                <form name="arrange" action="insert_loan.php" method="post" onSubmit="return validate();" enctype="multipart/form-data">
			...
			<tr>
			<td>Plan</td><td><select name="plans" id="plans" required class="form-control">
			    <option value="">--Select Plan--</option>
			    <?php   $plan=mysql_query("select * from `loan_plan`");
				    while($resplan=mysql_fetch_array($plan)){
			     ?>
								    <option value="<?php echo  $resplan['id'];?>"><?php echo $resplan['year'];?> Years</option>
								    <?php } ?>
								</select>
				    </td>
			<td>Rate of Intrest</td><td><input type="text" name="intrate" id="intrate"  required readonly class="form-control"></td>
			    ...
			    <td>Loan Amount</td><td><input type="text" name="loanamt" id="loanamt" onkeyPress="return IsNumeric(event,this.value)" required class="form-control"></td>
			</tr>
