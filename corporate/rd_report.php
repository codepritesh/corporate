    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Recurring Report</h2>
	    ...
	    <div class="box-content">
	     ...
	     <?php
                if(isset($_POST['submit']))
                {
				   ...
				   //echo "SELECT * FROM recurringdiposite WHERE  `status`=0 and `start_date` BETWEEN '$startdate' AND '$enddate'";
                    $fetch=mysql_query("SELECT * FROM recurringdiposite WHERE  `status`=0 and `start_date` BETWEEN '$startdate' AND '$enddate'");
                }else{
					//echo "select * from `recurringdiposite` where `status`=0 and `start_date`='$cdate' group by `id` desc";
				//$fetch=mysql_query("select * from `recurringdiposite` where `status`=0 and `start_date`='$cdate' group by `id` desc");
				$fetch=mysql_query("select * from `recurringdiposite` where `status`=0  group by `id` desc");
				}
                ?>
		<div id="print">
                <table class="table">
			...
			$tot=$res['deposited_amt']+$tot;
			    ...
			    <td><?php echo date("d-m-Y",strtotime($res['start_date']));?></td>
                        ...
                        <?php }?>
			<tr style="color: red;">
			    <td colspan="7" style="text-align: center;">Total Deposited Amount:</td><td colspan="4"><?php echo number_format($tot, 2, '.', '');?></td>
			</tr>
			<tr >
