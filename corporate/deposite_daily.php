...
//echo  "SELECT * FROM customer WHERE EXISTS (SELECT 1 FROM transaction WHERE transaction.customerid = customer.customer_id and transaction.date like '$currdate' and transaction.folio_no = '17' and transaction.productfolio = '4')";
$getvendor=mysql_query("SELECT * FROM customer WHERE EXISTS (SELECT 1 FROM transaction WHERE transaction.customerid = customer.customer_id and transaction.date like '$currdate'
					   and transaction.folio_no = '17' and transaction.productfolio = '4')")or die(mysql_error());
   ...
   while($resvendor=mysql_fetch_array($getvendor))
    ...
    $faccno=mysql_query("select max(id) as id from `dailydeposit`");
    $rfaccno=mysql_fetch_array($faccno);
    $raccno=$rfaccno['id']+1;
    
    $sqlagent=mysql_query("select * from `agent` where `agentcode_daily`!=''")or die(mysql_error());
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Create Daily Deposite</h2>
                <div class="box-icon">
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
                <form name="arrange" action="insert_daily.php" onSubmit="return validateform();" method="post" enctype="multipart/form-data">
                     <table style="float: left;width: 100%; height: 450px;">
					  <tr>
                        ...
                        <tr>
                            <td>A/c No</td><td><input type="text" name="accno" class="form-control" required></td>
			    <!--<td>A/c No</td><td><input type="text" name="accno" value="D<?php //echo  $raccno;?>" class="form-control" readonly="true" required></td>-->
			    ...
			    <td>DOB</td><td><input type="text" name="dob" id="dob" class="form-control" readonly="true" required></td>
                        ...
                        <tr>
			    <td>Phone No.</td><td><input type="text" name="phno" id="phno" class="form-control" readonly="true" required></td>
			    ...
			    <td>Dist</td><td><input type="text" name="dist" id="dist"  class="form-control" readonly="true" required></td>
                        ...
                        <tr>
			    <td>Annual income</td><td><input type="text" name="annual" autocomplete='off'  onKeyPress="return IsNumeric(event)" id="annual" class="form-control" required></td>
			    ...
			    <td>Nominee Relation</td><td><input type="text" name="nomrelation" autocomplete='off'  id="nomrelation" class="form-control"></td>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <select name="plan" id="plan" onChange="change(this.value)" class="form-control" required >
                                    <option value="">--select--</option>
                                    <?php
                            $fplan=mysql_query("select * from `dailydipositescheme` order by `timeperiod`");
                            while($resplan=mysql_fetch_array($fplan))
                            {
                            ?>
                                    <option value="<?php echo $resplan['interestrate'];?>"><?php echo  $resplan['timeperiod'];?></option>
                            <?php }?>
                                </select>
                            </td>
                            <td>Intrest rate</td><td><input type="text" name="intrest" id="intrest" class="form-control" readonly="true" required></td>
			</tr>
                        <tr>
			    <td>Daily Deposit Amount</td>
				          <td><select name="deposited_amt"  required class="form-control">
							    <option value="">--Select Amount--</option>
							    <?php $qwe=mysql_query("select * from `set_deposit_amt` where `scheme_id`='3'");
