   ...
   $table="badloan";
	 ...
	 while($resagent=mysql_fetch_array($sqlagent))
		...
		'idval' => $resagent['loan_accno'],
                'outstanding' => $resagent['amount_topay']
		);
		}
                
                
                $agent=mysql_query("select * from `agent`")or die(mysql_error());
                while($fetchagent=mysql_fetch_array($agent))
		{
		 $agentlist[] = array(
		'value'  =>$fetchagent['name'].'('.$fetchagent['codeno'].')',
		'idval' => $fetchagent['id']
		);
		}
?>
...
$(function(){
	 ...
	 select: function(event,ui){
		...
		var valshow=ui.item.value;
                $('#name').val(valshow);
		$('#accno').val(ui.item.idval);
                $('#outstanding').val(ui.item.outstanding);
                return false;
		}
        });
});
...
$(function(){
        var availabledrugs=<?php echo  json_encode($agentlist); ?>;
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>BadLoan Close</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
                <form name="arrange" action="badloancloseaction.php" onSubmit="return validateform();" method="post" enctype="multipart/form-data">
			...
			<tr>
			    ...
			    <td>
				<input type="text" name="outstanding" id="outstanding" onkeyPress="return IsNumeric(event,this.value)" class="form-control" readonly />
			    </td>
			  </tr>
                        <tr>
                            <td>Amount</td>
			    ...
			    <td>Agent</td>
			    <td>
							<input type="text" name="agentnm" id="agentnm" class="form-control"/>
			...
			<tr>
			    <td>Voucher No</td>
			    <td>
				<input type="text" name="voucher" id="voucher" required onkeyPress="return IsNumeric(event,this.value)" class="form-control" autocomplete="off"/>
			    </td>
