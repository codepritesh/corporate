    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Loan Index Report</h2>
	    ...
	    <div class="box-content">
			    ...
			    <tr>
                            ...
                            <option value="23">Staff Loan</option>					
            ...
            if($_POST['product']==0 || $_POST['product']==''){
               $fetch=mysql_query("SELECT * FROM transaction WHERE `amount`<'0' and `folio_no` IN('6','7','8','9','10','11','12','23') and (`type`='loan' or `details` NOT LIKE '%Refund%') and `date` BETWEEN '$sdate' AND '$edate'");
		    }else{
                $fetch=mysql_query("SELECT * FROM transaction WHERE `amount`<'0' and `folio_no`='$_POST[product]' and (`type`='loan' or `details` NOT LIKE '%Refund%') and `date` BETWEEN '$sdate' AND '$edate'");
		    }
			}else
		    {
		    $date=date("Y-m-d");
            $fetch=mysql_query("select * from transaction WHERE `amount`<'0' and `folio_no` IN('6','7','8','9','10','11','12','23') and (`type`='loan' or `details` NOT LIKE '%Refund%') and `date`='$date' ");
		    }
                    $slno=0;
                    $total=0;
                       while($res=mysql_fetch_array($fetch))
                       {
                        ...
                        $udet=mysql_fetch_array(mysql_query("select * from $table where `loan_accno`='$res[accountno]'"));
                        $agent=mysql_fetch_array(mysql_query("select * from `agent` where `id`='$udet[agent_id]'"));
                    
                        $sec=mysql_query("select * from  $udet[loan_against_acc] where `acc_no`='$udet[loan_against_accno]' ");
                    ...
                    <tr>
                        ...
                        <td><?php if($udet['loan_against_acc']=='compulsarydeposite' || $udet['loan_against_acc']=='dailydeposit'){echo $ressec['total_amt'];   }else{ echo $ressec['deposited_amt']; } ?></td>
                        <td><?php echo  $agent['name'];?></td>
                    </tr>
                    <?php } ?>
                    <tr style="color: red;"><td colspan="8">Total Withdrawl:</td><td colspan="3"><?php echo $total;?></td></tr>
		    <tr>
			<td colspan="11" style="text-align: center;"><input type="button" value="Print" onclick="printContent('print')" /></td>
		    </tr>
                </table>
	    </div>
	</div>
