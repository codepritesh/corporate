...
$saccdetails=mysql_query("select * from `dailydeposit` where `status`='0'")or die(mysql_error());
...
$(function(){
	...
	//alert(cid);
		...
		var date=new Date().toJSON().slice(0,10);
		 ...
		 else{alert("Maturity not done"); document.getElementById('s_submit').style.display='none'; return false; }
		}
	    });
     });
});
...
function validate() {
	...
	if (parseFloat(wamt) > parseFloat(damt))
	{
	    ...
	    var con1=confirm("You hanve not sufficient balance to withdraw "+wamt+" amount");
	    if(con1)
		{
...
<script>
	function addinter()
	{
		var deposited_amt=document.getElementById("deposited_amt").value;
		var intrest_amt=document.getElementById("intrest_amt").value;
		var total=parseFloat(deposited_amt)+parseFloat(intrest_amt);
		document.getElementById("with_amt").value=total;
	}
</script>
<html lang="en">
<head>
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Daily Account Withdrawal Form</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
                <form name="arrange" action="dailywith_action.php" method="post" onsubmit="return validate();" >
			...
			<tr>
			    ...
			    <td>Total Deposited Amount</td><td><input type="text" name="deposited_amt" id="deposited_amt" class="form-control"  ></td>
                             <input type="hidden" name="account" id="account">
				<input type="hidden" name="customer_id" id="customer_id">
			</tr>
                        <tr style="display: none;">
                            <td>Agent</td><td><input type="text" name="agent" id="agent" class="form-control"></td>
			    <input type="hidden" name="agentid" id="agentid">
			</tr>
			<tr>
			    <td>Intrest Amount</td><td><input type="text" name="intrest_amt" id="intrest_amt" class="form-control" onblur="return addinter();" required></td>
			    <td>Voucher no</td><td><input type="text" name="voucher" id="voucher" onKeyPress="return numbersonly(event);"  class="form-control" required ></td>
			</tr>
			<tr>
				<td>Withdrawal Amount</td><td><input type="text" name="with_amt" id="with_amt" class="form-control" onkeyPress="return IsNumeric(event,this.value)" required></td>
			   ...
			   <td><input type="text" name="chqddno" id="d1" class="form-control"  placeholder="Cheque/DD Number" onKeyPress="return numbersonly(event)" style="display: none;"></td>
			    ...
			    <td>&nbsp;</td><td>&nbsp;</td>
                            <td align="center" colspan="2"><input type="submit" id="s_submit" name="s_submit" value="Submit" /></td>
