...
$getvendor=mysql_query("SELECT * FROM loan where `is_approved`='1' and  `loan_accno`!='' and `loancomplited`='0'") or die(mysql_error());
...
<script type="text/javascript"> 
	...
	//alert(cid);
        $.ajax({
            url:"loan_refund_ajax.php?cid="+cid,
            success: function(result){
               // alert(result);
		var obj = JSON.parse(result);
               //alert(obj.value.intrest_amt);
		$('#loan').val(obj.value);
...
<script>
	...
	$('#refund').blur(function(){
		...
		if (parseFloat(intamt)>parseFloat(camt)) {
		    alert("You paid more amount than "+camt);
		    //$('#refund').val('');
		   // $('#refund').focus();
		    //return false;
		}
   ...
   if (intamt!="")
   {
    ...
    function caltotal()
    {
	var intrest=document.getElementById("intamt").value;
	var refund=document.getElementById("refund").value;
	if (intrest=='') { intrest=0;}
	if (refund=='') { refund=0;}
	var total=parseFloat(intrest)+parseFloat(refund);
	//alert(total);
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-plus"></i>Loan Repayment</h2>
            ...
            <div class="box-content" style="width:100%; float:left;height: auto;background-color: #fff;border: #f5f5f5 solid 0.5px; border-radius: 4px;-webkit-box-shadow: 0 0 10px rgba(189,189,189,.4);">
                <form name="arrange" action="loanrefund_action.php" method="post" onSubmit="return validate();" enctype="multipart/form-data">		
		   <table style="float: left;width: 100%;">
                        <tr>
                            <td colspan="4" style="color: red;font-size: 20px;"><?php if(isset($_GET['msg'])){ echo $_GET['msg']; }  ?></td>
                        </tr>
			<tr>
                            <td>Customer_id</td><td style="width:310px;"><select name="loan" data-placeholder="&nbsp;"  class="form-control chosen-select " id="customer" style="width:307px; padding:4px;height: 100px;border-radius:4px;">
										<option value=""></option>
										<?php echo $custmerid;?>
									</select>
			...
			<tr>
			    <td>OD Amount</td><td><input type="text" name="odprincipal" class="form-control" id="odprincipal" required readonly></td>
