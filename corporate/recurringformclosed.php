...
$getrecurring=mysql_query("select * from `recurringdiposite` where `acc_no`='$_GET[acc_no]'")or die(mysql_error());
   ...
   $(function(){
	...
	//alert(cid);
		...
		var obj = JSON.parse(result);
		 ...
		 else if (obj.installment>7) {
		    alert("your account has been closed");
		    window.location.href = "recurringformclosed.php";
		 }else{
		 $('#fine').val(obj.fine);
		 }
            }
...
return false //disable key press
  ...
  <script>
    ...
    if (sel==3) { var amount=fmamt*3; if(amount<=acam){$('#installment').val(3); $('#amount').val(amount); var finee=(amount/100)*12; $('#fine').val(finee); var tot=parseFloat(amount)+parseFloat(finee); $('#tamount').val(tot);}}
    if (sel==4) { var amount=fmamt*4; if(amount<=acam){$('#installment').val(4); $('#amount').val(amount); var finee=(amount/100)*24; $('#fine').val(finee); var tot=parseFloat(amount)+parseFloat(finee); $('#tamount').val(tot);}}
    if (sel==5) { var amount=fmamt*5; if(amount<=acam){$('#installment').val(5); $('#amount').val(amount); var finee=(amount/100)*48; $('#fine').val(finee); var tot=parseFloat(amount)+parseFloat(finee); $('#tamount').val(tot);}}
    if (sel==6) { var amount=fmamt*6; if(amount<=acam){$('#installment').val(6); $('#amount').val(amount); var finee=(amount/100)*96; $('#fine').val(finee); var tot=parseFloat(amount)+parseFloat(finee); $('#tamount').val(tot);}}
    ...
    function sumtotal()
    { 
	var fetamt=document.getElementById("amount").value;
	var insfine=document.getElementById("fine").value;
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            ...
            <div class="box-content">
                 <form name="arrange" action="recurringformclosed_action.php" onSubmit="return validate();" method="post" enctype="multipart/form-data">
		    <table style="float: left;width: 100%;">
                            <span style="color: red;font-size: 20px;"><?php if(isset($_GET['msg'])){ echo $_GET['msg']; }  ?></span>
			<tr>
                             <td>Customer_id</td><td style="width:310px;">
				<input type="text" name="customer" id="customer" value="<?php echo $resrecurring['acc_no']."(".$resrecurring['name'].")"; ?>" readonly="true" class="form-control" required>
                             </td>
			    <input type="hidden" name="acid" id="acid" value="<?php echo $resrecurring['acc_no'];?>">
			    <input type="hidden" name="cuid" id="cuid" value="<?php echo $resrecurring['customer_id'];?>">
                            <td>Monthly Amount</td><td><input type="text" name="mamount" value="<?php echo $resrecurring['monthly_amount'];?>" id="mamount" readonly="true" class="form-control" required></td>
			</tr>
			<tr>
			    <td>Amount</td><td><input type="text" name="amount" id="amount" class="form-control" required></td>
			    ...
			    <input type="hidden" name="lastamount" id="lastamount" value="<?php echo $resrecurring['deposited_amt'];?>" class="form-control" required>
			<tr>
